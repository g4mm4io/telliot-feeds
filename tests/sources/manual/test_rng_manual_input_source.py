...
async def test_rng_valid_input(capsys):
    with mock.patch(
        ...
        result, _ = await FetchRNGManualInputSource().fetch_new_datapoint()
        expected = "FetchRNG value to be submitted on chain: 2b563420722cbcfc84857129bef775e0dc5f1401\nPress [ENTER] to continue"  # noqa: E501
...
async def test_rng_invalid_input(capsys):
    with mock.patch(
        ...
        _, _ = await FetchRNGManualInputSource().fetch_new_datapoint()
