""" Unit tests for FetchOracleAddress and AutopayAddresses queries
...
from telliot_feeds.queries.fetch.autopay_addresses import AutopayAddresses
from telliot_feeds.queries.fetch.fetch_oracle_address import FetchOracleAddress
...
def test_autopay_encode_decode_reported_val():
    ...
    # an address value indicating the current fetch autopay addresses
...
def test_oracle_address_constructor():
    ...
    query_type, encoded_param_vals = decode_abi(["string", "bytes"], q.query_data)
    assert query_type == "FetchOracleAddress"
